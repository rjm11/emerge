...
function ModuleManager:getModuleList()
  ...
  -- Add/override with custom modules (user-defined take precedence)
...
function ModuleManager:loadModule(module_id)
  ...
  -- Set up handler for download completion; ensure only one handler is active
  ...
  self.handlers.download = registerAnonymousEventHandler("sysDownloadDone", function(_, filename)
    if filename:find(module_id .. "%.lua$") then
      ...
      -- Lifecycle: preload
      pcall(function()
        raiseEvent("module.lifecycle.preload", { id = module_id })
      end)

      -- Load the module file into Lua and execute; modules should call EMERGE:register
      ...
      if ok then
        ...
        if not ok then
          ...
          -- Lifecycle: load
          pcall(function()
            raiseEvent("module.lifecycle.load", { id = module_id })
          end)
          -- Lifecycle: postload (deferred for stability)
          tempTimer(0.2, function()
            pcall(function()
              raiseEvent("module.lifecycle.postload", { id = module_id })
            end)
          end)
        end
      else
...
function ModuleManager:discoverRepositories()
  ...
  for _, repo_config in ipairs(self.repositories) do
    ...
    self:downloadManifest(repo_config, function(success, modules)
      ...
      -- When all repository downloads complete, finalize the cache snapshot
