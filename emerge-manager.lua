...
function ModuleManager:createPersistentLoader(forceCreate)
  ...
  if not io.exists(manager_file) then
    ...
    cecho("<yellow>[EMERGE] Creating bootstrap file...<reset>\n")
...
if io.exists(manager_file) then
  ...
  if script_id then
    ...
    -- Save the profile
    if saveProfile() then
      cecho("<green>[EMERGE] Profile saved<reset>\n")
    else
      cecho("<yellow>[EMERGE] Please save your profile manually (Ctrl+S)<reset>\n")
    end
  else
    cecho("<IndianRed>[EMERGE] Failed to create bootloader script<reset>\n")
    cecho("<yellow>[EMERGE] Please create manually in Script Editor:<reset>\n")
    cecho("<LightSteelBlue>1. Open Script Editor (Alt+E)<reset>\n")
    cecho("<LightSteelBlue>2. Create a script group called 'EMERGE'<reset>\n")
